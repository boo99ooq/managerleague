...
# CSS GOLDEN: Neretto 900 totale e stili card
st.markdown("""
<style>
    /* Forza neretto 900 su ogni elemento di testo e tabella */
...
# --- FUNZIONI SUPPORTO ---
def bold_style(df):
    """Applica il neretto 900 direttamente alle celle del DataFrame tramite Pandas Styler"""
...
with t[0]: # CLASSIFICHE + FM
    ...
    if f_pt is not None:
        with c1:
            ...
            st.dataframe(bold_style(f_pt[['Posizione','Giocatore','P_N','FM']].sort_values('Posizione')).background_gradient(subset=['P_N','FM'], cmap='YlGn').format({"P_N":"{:g}", "FM":"{:.2f}"}), hide_index=True, use_container_width=True)
    if f_sc is not None:
        with c2:
            st.subheader("⚔️ SCONTRI")
            f_sc['P_S'] = f_sc['Punti'].apply(to_num)
            st.dataframe(bold_style(f_sc[['Posizione','Giocatore','P_S']].sort_values('Posizione')).background_gradient(subset=['P_S'], cmap='Blues').format({"P_S":"{:g}"}), hide_index=True, use_container_width=True)
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        st.dataframe(bold_style(bu[['Squadra_N'] + sel + ['TOTALE']].sort_values('TOTALE', ascending=False)).background_gradient(cmap='YlOrRd', subset=['TOTALE']).format({c: "{:g}" for c in num_cols}), hide_index=True, use_container_width=True)

with t[2]: # ROSE COLORATE
    if f_rs is not None:
        c1, c2 = st.columns([1, 2])
        with c1: sq_r = st.selectbox("**SQUADRA**", ["TUTTE"] + sorted(f_rs['Squadra_N'].unique()))
        with c2: cerca_r = st.text_input("🔍 **CERCA**", "").upper()
        df_r = f_rs.copy()
        ...
        def color_ruolo(val):
            ...
            return ''
        st.dataframe(bold_style(df_r[['Ruolo', 'Nome', 'Prezzo_N', 'Quotazione']]).applymap(color_ruolo, subset=['Ruolo']).format({"Prezzo_N":"{:g}", "Quotazione":"{:g}"}), hide_index=True, use_container_width=True)

with t[4]: # SCAMBI DETTAGLIATI
    st.subheader("🔄 SCAMBI GOLDEN")
    if f_rs is not None:
        col1, col2 = st.columns(2)
        with col1: sA = st.selectbox("SQUADRA A", sorted(f_rs['Squadra_N'].unique()), key="sA_new"); gA = st.multiselect("ESCONO DA A", f_rs[f_rs['Squadra_N']==sA]['Nome'].tolist())
        with col2: sB = st.selectbox("SQUADRA B", [s for s in sorted(f_rs['Squadra_N'].unique()) if s != sA], key="sB_new"); gB = st.multiselect("ESCONO DA B", f_rs[f_rs['Squadra_N']==sB]['Nome'].tolist())
        ...
        if gA and gB:
            ...
            cp1.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sA}<br><h2>{int(pA+diff)}</h2><small>PRIMA: {int(pA)}</small></div>', unsafe_allow_html=True)
            cp2.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sB}<br><h2>{int(pB-diff)}</h2><small>PRIMA: {int(pB)}</small></div>', unsafe_allow_html=True)

with t[5]: # TAGLI GIGANTI
    st.subheader("✂️ TAGLI GOLDEN")
    if f_rs is not None:
        sq_t = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="st_t_final")
...
with t[6]: # MERCATO (FIXED SYNTAX)
    ...
    if not df_mercato.empty:
        for idx, row in df_mercato.iterrows():
            ...
            with mc1: 
                st.write(f"**{row['GIOCATORE']}** ({row['SQUADRA']})")
            with mc2: 
                st.write(f"TOT: {row['TOTALE']:g}")
            with mc3: 
                cl = 'status-ufficiale' if row['STATO'] == 'UFFICIALE' else 'status-probabile'
                st.write(f"<span class='{cl}'>{row['STATO']}</span>", unsafe_allow_html=True)
            with mc4:
        ...
        for i, (sq_n, t_n) in enumerate(sq_m.items()):
            with cols_m[i % 4]: 
                st.markdown(f'<div class="refund-box"><small>{sq_n}</small><br><b style="font-size:1.5em;">+{t_n:g}</b></div>', unsafe_allow_html=True)
