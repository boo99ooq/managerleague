...
# CSS INTEGRALE (Unione stili App + Lab)
st.markdown("""
<style>
...
# --- DB MERCATO (RICALCOLO E AUTO-FIX) ---
...
if os.path.exists(FILE_DB):
    ...
    except: df_mercato = pd.DataFrame(columns=cols_mercato)
else:
...
with st.sidebar:
    ...
    if f_rs is not None:
        ...
        for n in scelte:
            ...
            st.markdown(f'''<div class="player-card" style="background-color: {bg};">
                <b style="font-size: 1.1em;">{n}</b><br><small>({dr["Squadra_N"]})</small><hr style="margin: 8px 0; border: 0; border-top: 2px solid #333;">
                ASTA: <b>{dr["Prezzo_N"]:g}</b> | VINC: <b>{vv:g}</b><br>QUOT: <b style="color: #1a73e8;">{dr["Quotazione"]:g}</b>
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        sel_b = st.multiselect("**VOCI PATRIMONIO:**", voci_disp, default=voci_disp)
        bu['PATRIMONIO TOTALE'] = bu[sel_b].sum(axis=1) if sel_b else 0
        st.dataframe(bu[['Squadra_N'] + sel_b + ['PATRIMONIO TOTALE']].sort_values("PATRIMONIO TOTALE", ascending=False).style.background_gradient(cmap='YlOrRd', subset=['PATRIMONIO TOTALE']).format({c: "{:g}" for c in bu.columns if c != 'Squadra_N'}), hide_index=True, use_container_width=True)
...
with t[4]: # SCAMBI
    ...
    if f_rs is not None:
        ...
        if ga and gb:
            ...
            st.markdown(f'<div class="punto-incontro-wrapper"><div class="punto-incontro-box"><small>Incontro</small><br><b style="color:{col_g};font-size:1.2em;">GAP: {gap:g}</b><br><small>Media: {nuovo_tot:g}</small></div></div>', unsafe_allow_html=True)
            r1, r2 = st.columns(2)
            with r1:
