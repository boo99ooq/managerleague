...
# CSS GOLDEN (Neretto 900 totale, Card, Box e tabelle blindate)
st.markdown("""
<style>
    /* Forza il neretto 900 su ogni elemento, comprese le celle delle tabelle */
...
with t[1]: # BUDGET DINAMICO (RIPRISTINATO)
    if f_rs is not None:
...
with t[2]: # ROSE CON COLORAZIONE PER RUOLO
    if f_rs is not None:
...
with t[4]: # SCAMBI CON PATRIMONI (PRIMA VS DOPO)
    ...
    if f_rs is not None:
        ...
        if gA and gB:
            def get_v(n):
                ...
                return {'t': p+v, 'v': v}
            da, db = {n: get_v(n) for n in gA}, {n: get_val(n) for n in gB} if 'get_val' in locals() else {n: get_v(n) for n in gB}
...
with t[5]: # TAGLI GIGANTI
    ...
    if f_rs is not None:
        ...
        if gt:
            ...
            st.markdown(f'''<div class="cut-box"><div class="cut-player-name">{gt}</div><div style="font-size:2em; color:#2e7d32;">RIMBORSO: {round((v_asta+vv)*0.6):g} CREDITI</div><br>ASTA: {v_asta:g} | VINCOLI: {vv:g}</div>''', unsafe_allow_html=True)

with t[6]: # MERCATO GENNAIO (CON RIEPILOGO RIMBORSI)
    ...
    if not df_mercato.empty:
        for idx, row in df_mercato.iterrows():
            ...
            with mc3: st.write(f"<span class='status-probabile'>{row['STATO']}</span>" if row['STATO']=="PROBABILE" else f"<span class='status-ufficiale'>{row['STATO']}</span>", unsafe_allow_html=True)
            with mc4:
        ...
        cols_m = st.columns(4)
        for idx, (sq_name, sq_tot) in enumerate(sq_m.items()):
            with cols_m[idx % 4]:
                st.markdown(f'<div class="refund-box"><small>{sq_name}</small><br><b style="font-size:1.5em;">+{sq_tot:g}</b></div>', unsafe_allow_html=True)
