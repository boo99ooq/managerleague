...
# 1. SETUP UI
st.set_page_config(page_title="MuyFantaLAB - Golden Full", layout="wide", initial_sidebar_state="expanded")

# CSS INTEGRALE
st.markdown("""
<style>
    html, body, [data-testid="stAppViewCimport streamlit as st
import pandas as pd
import os
import unicodedata
import re
...
# CSS ORIGINALE + STILI MERCATO (Grassetto estremo, card e box personalizzati)
st.markdown("""
<style>
        ...
        for i, row in df_mercato.iterrows():
            if row.get('SPESA', 0) == 0:
                ...
                if not match.empty:
                    ...
                    rb_v = (s_v + q_v) * 0.5
                    df_mercato.at[i, 'SPESA'], df_mercato.at[i, 'QUOT'] = s_v, q_v
                    df_mercato.at[i, 'RIMB_BASE'], df_mercato.at[i, 'VINCOLO'] = rb_v, vv_v
...
with st.sidebar:
    ...
    if f_rs is not None:
        ...
        for n in scelte:
            ...
            if 'POR' in r or r == 'P': bg_side = '#FCE4EC'
            elif 'DIF' in r or r == 'D': bg_side = '#E8F5E9'
            elif 'CEN' in r or r == 'C': bg_side = '#E3F2FD'
            elif 'ATT' in r or r == 'A': bg_side = '#FFFDE7'
            else: bg_side = '#f1f3f4'
...
with t[0]: # CLASSIFICHE
    ...
    if f_sc is not None:
        with c2:
            ...
            st.dataframe(f_sc[['Posizione','Giocatore','P_S','GF','GS']].style.background_gradient(subset=['P_S'], cmap='Blues').format({"P_S": "{:g}", "GF": "{:g}", "GS": "{:g}"}).set_properties(**{'font-weight': '900'}), hide_index=True, use_container_width=True)
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        st.bar_chart(bu.set_index("Squadra_N")[['SPESA ROSE', 'SPESA VINCOLI', 'CREDITI DISPONIBILI', 'RECUPERO CESSIONI']], color=["#1a73e8", "#9c27b0", "#ff9800", "#2e7d32"])
...
with t[2]: # ROSE
    if f_rs is not None:
        ...
        if len(mancanti) > 0: st.markdown(f'<div class="zero-tool">⚠️ {len(mancanti)} NON RICONOSCIUTI: {", ".join(mancanti)}</div>', unsafe_allow_html=True)
...
with t[3]: # VINCOLI
    ...
    if f_vn is not None:
        ...
        st.dataframe(df_v_disp[['Squadra', 'Giocatore', 'Tot_Vincolo', 'Anni_T']].sort_values('Tot_Vincolo', ascending=False).style.background_gradient(subset=['Tot_Vincolo'], cmap='Purples').format({"Tot_Vincolo": "{:g}"}).set_properties(**{'font-weight': '900'}), hide_index=True, use_container_width=True)
...
with t[4]: # SCAMBI
    ...
    if f_rs is not None:
        ...
        if ga and gb:
            def get_i(n):
                p = f_rs[f_rs['Nome']==n]['Prezzo_N'].iloc[0]; v_r = f_vn[f_vn['Giocatore_Match']==super_clean_match(n)] if f_vn is not None else pd.DataFrame(); v = v_r['Tot_Vincolo'].iloc[0] if not v_r.empty else 0
                return {'t': p + v, 'v': v}
            ...
            st.divider(); m1, m2 = st.columns(2); m1.metric(f"Valore ceduto da {sa}", f"{int(tot_a)}"); m2.metric(f"Valore ceduto da {sb}", f"{int(tot_b)}")
            ...
            st.divider(); p_a = f_rs[f_rs['Squadra_N']==sa]['Prezzo_N'].sum() + (f_vn[f_vn['Sq_N']==sa]['Tot_Vincolo'].sum() if f_vn is not None else 0) + bg_ex.get(sa, 0)
            ...
            col_p1.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sa}<br><h2>{int(p_a + diff)}</h2><small>PRIMA: {int(p_a)}</small></div>', unsafe_allow_html=True)
            col_p2.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sb}<br><h2>{int(p_b - diff)}</h2><small>PRIMA: {int(p_b)}</small></div>', unsafe_allow_html=True)

with t[5]: # TAGLI
    st.subheader("✂️ **SIMULATORE TAGLI**")
    sq_t = st.selectbox("**SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="sq_tag"); gi_t = st.selectbox("**GIOCATORE**", f_rs[f_rs['Squadra_N'] == sq_t]['Nome'].tolist(), key="gioc_tag")
    if gi_t:
        v_p = f_rs[(f_rs['Squadra_N'] == sq_t) & (f_rs['Nome'] == gi_t)]['Prezzo_N'].iloc[0]
        v_m = f_vn[f_vn['Giocatore_Match'] == super_clean_match(gi_t)] if f_vn is not None else pd.DataFrame(); v_v = v_m['Tot_Vincolo'].iloc[0] if not v_m.empty else 0
        st.markdown(f'<div class="cut-box"><h3>💰 **RIMBORSO: {round((v_p + v_v) * 0.6)} CREDITI**</h3>VALUTAZIONE: <b>{int(v_p)}</b> | VINC: <b>{int(v_v)}</b></div>', unsafe_allow_html=True)

with t[6]: # NUOVA TAB: RIMBORSO CESSIONI
    ...
    if not df_mercato.empty:
        st.write("---"); h1, h2, h3, h4, h5, h6 = st.columns([2, 1, 1, 2, 1, 1]); h1.write("**GIOCATORE**"); h2.write("**SPESA**"); h3.write("**QUOT**"); h4.write("**DETTAGLIO**"); h5.write("**TOTALE**"); h6.write("**STATO**")
        for i, row in df_mercato.iterrows():
        ...
        st.write("---"); st.markdown("### 💰 RIEPILOGO RIMBORSI PER SQUADRA")
        ...
        for idx, (squadra, data) in enumerate(sq_stats.iterrows()):
            with cols[idx % 4]: st.markdown(f'<div class="refund-box"><small>{squadra}</small><br><b>+{data["TOT_GEN"]:g}</b><br><hr style="margin:5px 0; border:0; border-top:1px solid #ccc;"><span class="text-ufficiale">Uff: {data["UFFICIALE"]:g}</span><br><span class="text-probabile">Prob: {data["PROBABILE"]:g}</span></div>', unsafe_allow_html=True)ontainer"] * { font-weight: 900 !important; }
...
if os.path.exists(FILE_DB):
    df_mercato = pd.read_csv(FILE_DB)
    # Auto-fix colonne mancanti
    if not df_mercato.empty and f_rs is not None:
        for i, row in df_mercato.iterrows():
            if row.get('SPESA', 0) == 0:
                m = f_rs[f_rs['Nome'] == row['GIOCATORE']]
                if not m.empty:
                    s, q = m.iloc[0]['Prezzo_N'], m.iloc[0]['Quotazione']
                    v_m = f_vn[f_vn['Giocatore_Match'] == super_clean_match(row['GIOCATORE'])] if f_vn is not None else pd.DataFrame()
                    vv = v_m['Tot_Vincolo'].iloc[0] if not v_m.empty else 0
                    rb = (s + q) * 0.5
...
with st.sidebar:
    ...
    if f_rs is not None:
        ...
        for n in cerca:
            ...
            vv = v_row['Tot_Vincolo'].iloc[0] if not v_row.empty else 0
            r = str(dr['Ruolo']).upper()
...
with t[0]:
    ...
    if f_pt is not None:
        with c1:
            st.subheader("🎯 CLASSIFICA PUNTI")
            f_pt['P_N'] = pd.to_numeric(f_pt['Punti Totali'], errors='coerce').fillna(0)
    ...
    if f_sc is not None:
        with c2:
            st.subheader("⚔️ SCONTRI DIRETTI")
            f_sc['P_S'] = pd.to_numeric(f_sc['Punti'], errors='coerce').fillna(0)
...
with t[2]:
    if f_rs is not None:
        sq_sel = st.selectbox("**SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="rose_sel")
        ...
        def color_row(row):
            ...
            return [f'background-color: {bg}; color: black; font-weight: 900;'] * len(row)
        st.dataframe(df_sq.style.apply(color_row, axis=1).format({"Prezzo_N":"{:g}", "Quotazione":"{:g}"}), hide_index=True, use_container_width=True)
...
with t[4]:
    ...
    if f_rs is not None:
        ...
        with sc1: sa = st.selectbox("SQUADRA A", sorted(f_rs['Squadra_N'].unique()), key="sa"); ga = st.multiselect("DA A", f_rs[f_rs['Squadra_N']==sa]['Nome'].tolist())
        with sc2: sb = st.selectbox("SQUADRA B", sorted([s for s in f_rs['Squadra_N'].unique() if s != sa]), key="sb"); gb = st.multiselect("DA B", f_rs[f_rs['Squadra_N']==sb]['Nome'].tolist())
        if ga and gb:
            def get_val(n):
            ...
            with r1:
                for n, i in db.items():
                    ps = i['t']/tb if tb > 0 else 1/len(gb)
                    st.markdown(f'<div class="player-card" style="background-color: #e3f2fd;"><b>{n}</b><br>VAL: {int(max(0, round(ps*nt)-i["v"]))}</div>', unsafe_allow_html=True)
            with r2:
                for n, i in da.items():
                    ps = i['t']/ta if ta > 0 else 1/len(ga)
                    st.markdown(f'<div class="player-card" style="background-color: #fbe9e7;"><b>{n}</b><br>VAL: {int(max(0, round(ps*nt)-i["v"]))}</div>', unsafe_allow_html=True)

# --- TAB 5: TAGLI ---
with t[5]:
    st.subheader("✂️ SIMULATORE TAGLI")
    if f_rs is not None:
        sqt = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sqt")
        gt = st.selectbox("GIOCATORE", f_rs[f_rs['Squadra_N'] == sqt]['Nome'].tolist())
        if gt:
            p = f_rs[f_rs['Nome'] == gt]['Prezzo_N'].iloc[0]
            vm = f_vn[f_vn['Giocatore_Match'] == super_clean_match(gt)] if f_vn is not None else pd.DataFrame()
            v = vm['Tot_Vincolo'].iloc[0] if not v_m.empty else 0
            st.markdown(f'<div style="background-color:#fff3e0; padding:20px; border-radius:10px; border:2px solid #ff9800;"><h3>💰 RIMBORSO TAGLIO (60%): {int(round((p + v) * 0.6))}</h3></div>', unsafe_allow_html=True)
...
with t[6]:
    ...
    if not df_mercato.empty:
        ...
        for idx, (squadra, data) in enumerate(sq_stats.iterrows()):
            with cols[idx % 4]:
                st.markdown(f'<div class="refund-box"><small>{squadra}</small><br><span style="font-size: 1.2em;"><b>+{data["TOTALE_GENERALE"]:g}</b></span><br><hr style="margin: 5px 0; border: 0; border-top: 1px solid #ccc;"><span class="text-ufficiale">Uff: {data["UFFICIALE"]:g}</span><br><span class="text-probabile">Prob: {data["PROBABILE"]:g}</span></div>', unsafe_allow_html=True)
