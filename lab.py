...
# CSS DEFINITIVO: Forza il neretto 900 su TUTTO (Tabelle, Sidebar, Input)
st.markdown("""
<style>
    /* Regola per tutto il testo visibile */
    html, body, [data-testid="stAppViewContainer"] *, .stDataFrame td, .stDataFrame th, p, div, span { 
...
with st.sidebar:
    ...
    if f_rs is not None:
        ...
        for n in cerca_side:
            ...
            st.markdown(f'''<div class="player-card" style="background-color: #f1f3f4;"><b>{n}</b> ({d_g['Squadra_N']})<br>ASTA: {int(d_g['Prezzo_N'])} | VINC: {int(vv)}<br>QUOT: {int(d_g['Quotazione'])}</div>''', unsafe_allow_html=True)
...
with t[4]: # SCAMBI
    ...
    if f_rs is not None:
        ...
        if gA and gB:
            ...
            cp1.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sA}<br><h2>{int(pA+d_p)}</h2><small>PRIMA: {int(pA)}</small></div>', unsafe_allow_html=True)
            cp2.markdown(f'<div class="patrimonio-box">NUOVO PATRIMONIO {sB}<br><h2>{int(pB-d_p)}</h2><small>PRIMA: {int(pB)}</small></div>', unsafe_allow_html=True)

with t[5]: # TAGLI GIGANTI
    st.subheader("✂️ TAGLI GOLDEN")
    if f_rs is not None:
        sq_t = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sq_tag_final")
        ...
        if gt:
            ...
            st.markdown(f'''<div class="cut-box"><div class="cut-player-name">{gt}</div><div style="font-size:2em; color:#2e7d32;">RIMBORSO: {round((v_asta+vv)*0.6):g}</div><br>ASTA: {v_asta:g} | VINCOLI: {vv:g}</div>''', unsafe_allow_html=True)
