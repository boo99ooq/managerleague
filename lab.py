...
# CSS DEFINITIVO (Forza il neretto 900 su TUTTE le celle delle tabelle)
st.markdown("""
<style>
    /* Regola globale per il testo */
    html, body, [data-testid="stAppViewContainer"] * { 
        font-weight: 900 !important; 
        color: #000 !important; 
    }
    /* Regola specifica per le tabelle Streamlit (Celle e Intestazioni) */
...
with t[0]: # CLASSIFICHE CON MEDIA PUNTI
    c1, c2 = st.columns(2)
    if f_pt is not None:
        with c1:
            st.subheader("🎯 PUNTI")
...
with t[5]: # TAGLI GIGANTI
    ...
    if f_rs is not None:
        sq_t = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sq_t_tag")
