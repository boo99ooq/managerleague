    ...
    if f_rs is not None:
        ...
        if f_qt is not None:
            ...
            if c_v_qt:
                ...
                f_rs['Quotazione'] = f_rs[c_v_qt].apply(to_num).fillna(0)
...
with t[1]: # BUDGET (FIX VALUE ERROR)
    if f_rs is not None:
        ...
        if sel:
            ...
            # FIX: Formattazione selettiva solo sulle colonne numeriche
            st.dataframe(bold_df(bu[['Squadra_N'] + sel + ['TOTALE']]).background_gradient(subset=['TOTALE'], cmap='YlOrRd').format({c: "{:g}" for c in sel + ['TOTALE']}), hide_index=True, use_container_width=True)
...
with t[6]: # MERCATO (Box Pastello)
    ...
    if not df_mercato.empty:
        ...
        sq_m['TOT'] = sq_m.get('UFFICIALE',0) + sq_m.get('PROBABILE',0); colori = ["bg-azzurro", "bg-verde", "bg-rosa", "bg-giallo"]
        cols = st.columns(4)
