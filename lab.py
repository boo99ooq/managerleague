...
<style>
    ...
    .refund-box { background-color: #e8f5e9; padding: 15px; border-radius: 10px; border: 2px solid #2e7d32; color: #1b5e20; margin-bottom: 10px; border: 2px solid #2e7d32; }
</style>
...
# --- CALCOLO RIMBORSI GLOBALE (UFFICIALI + SIMULATI) ---
...
# Calcolo simulati dallo stato della sessione
r_sim = {}
...
with t[6]: # RIMBORSO CESSIONI
    ...
    with col1:
        ...
        scelta_uff = st.selectbox("Giocatore partente:", [""] + sorted(f_rs['Nome'].unique()), key="uff_sel")
    ...
    with col2:
        ...
        st.session_state.simulati = st.multiselect("Simula cessione per:", sorted(f_rs['Nome'].unique()), default=st.session_state.simulati)
        st.info("I nomi nel simulatore aggiornano il budget ma non vengono salvati nel file.")
