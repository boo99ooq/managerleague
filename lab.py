...
# CSS TOTALE: Forza il neretto 900 su TUTTO l'applicativo, tabelle incluse
st.markdown("""
<style>
    /* Regola globale per ogni testo, paragrafo, input e componente */
    html, body, [data-testid="stAppViewContainer"] *, .stMarkdown p, label, .stSelectbox div { 
        font-weight: 900 !important; 
        color: #000 !important; 
    }
    /* Regola specifica per forzare il grassetto nelle celle delle tabelle (td) e intestazioni (th) */
...
def bold_df(df):
    """Forza il grassetto nello stile della tabella"""
...
with st.sidebar:
    st.header("🔍 **CERCA NELLA LEGA**")
    if f_rs is not None:
        scelta = st.multiselect("**GIOCATORE**", sorted(f_rs['Nome'].unique()))
        for n in scelta:
            dg = f_rs[f_rs['Nome'] == n].iloc[0]
            ...
            st.markdown(f'''<div class="player-card"><b>{n}</b> ({dg['Squadra_N']})<br>ASTA: {int(dg['Prezzo_N'])} | VINC: {int(vv)}<br>QUOT: {int(dg['Quotazione'])}</div>''', unsafe_allow_html=True)
...
with t[0]: # CLASSIFICHE
    ...
    if f_sc is not None:
        with c2:
            ...
            st.dataframe(bold_df(f_sc[['Posizione','Giocatore','P_S']].sort_values('Posizione')).background_gradient(subset=['P_S'], cmap='Blues').format({"P_S":"{:g}"}), hide_index=True, use_container_width=True)
...
with t[1]: # BUDGET DINAMICO
    if f_rs is not None:
        ...
        sel = st.multiselect("**FILTRA PATRIMONIO:**", voci, default=voci)
...
with t[2]: # ROSE COLORATE
    if f_rs is not None:
        ...
        with c1: sq_r = st.selectbox("**SQUADRA**", ["TUTTE"] + sorted(f_rs['Squadra_N'].unique()), key="sq_rose")
        with c2: cerca_r = st.text_input("🔍 **CERCA GIOCATORE**", "", key="cerca_rose").upper()
        df_r = f_rs.copy()
...
with t[6]: # MERCATO CESSIONI (LAB)
    st.subheader("🚀 MERCATO CESSIONI ESTERE")
    ...
    if not df_mercato.empty:
        for idx, row in df_mercato.iterrows():
            ...
            with mc2: st.write(f"TOT: {row['TOTALE']:g}"); with mc3: st.write(f"<span class='{'status-ufficiale' if row['STATO']=='UFFICIALE' else 'status-probabile'}'>{row['STATO']}</span>", unsafe_allow_html=True)
            with mc4:
        ...
        st.write("---")
        sq_m_stats = df_mercato.groupby('SQUADRA')['TOTALE'].sum().sort_values(ascending=False)
        cols_m = st.columns(4)
        for i, (sq_n, t_n) in enumerate(sq_m_stats.items()):
