...
# CSS INTEGRALE (Bordi netti, ombre e layout Golden)
st.markdown("""
<style>
    html, body, [data-testid="stAppViewContainer"] * { font-weight: 900 !important; }
    .player-card { 
        padding: 12px; border-radius: 10px; margin-bottom: 12px; 
        border: 3px solid #333; box-shadow: 4px 4px 8px rgba(0,0,0,0.2); color: black; 
    }
    ...
    .punto-incontro-box { 
        background-color: #fff3e0; padding: 8px 25px; border-radius: 12px; 
        border: 3px solid #ff9800; text-align: center; width: fit-content; 
    }
...
# --- DB MERCATO (RICALCOLO AUTOMATICO) ---
        ...
        for i, row in df_mercato.iterrows():
            if row.get('SPESA', 0) == 0:
                ...
                if not match.empty:
                    ...
                    rb_v = (s_v + q_v) * 0.5
                    df_mercato.at[i, 'SPESA'], df_mercato.at[i, 'QUOT'], df_mercato.at[i, 'RIMB_BASE'], df_mercato.at[i, 'VINCOLO'], df_mercato.at[i, 'TOTALE'] = s_v, q_v, rb_v, vv_v, rb_v+vv_v
else:
...
with t[0]: # CLASSIFICHE
    ...
    if f_sc is not None:
        with c2:
            ...
            st.dataframe(f_sc[['Posizione','Giocatore','P_S','GF','GS','DR']].sort_values('Posizione').style.background_gradient(subset=['P_S'], cmap='Blues'), hide_index=True, use_container_width=True)
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        st.dataframe(bu[['Squadra_N'] + sel_budget + ['PATRIMONIO TOTALE']].sort_values("PATRIMONIO TOTALE", ascending=False).style.background_gradient(cmap='YlOrRd', subset=['PATRIMONIO TOTALE']), hide_index=True, use_container_width=True)

with t[4]: # SCAMBI (CON STATS GOLDEN)
    ...
    if f_rs is not None:
        ...
        if ga and gb:
            ...
            txt_g = f"GAP: +{int(gap)} (A > B)" if gap > 0 else f"GAP: +{int(abs(gap))} (B > A)" if gap < 0 else "EQUILIBRIO"
            st.markdown(f'<div class="punto-incontro-wrapper"><div class="punto-incontro-box"><small>Incontro</small><br><b style="color:{col_g};font-size:1.2em;">{txt_g}</b><br><small>Media: {nuovo_tot}</small></div></div>', unsafe_allow_html=True)

            res_a, res_b = st.columns(2)
            with res_a:
                for n, i in dict_b.items():
                    ps = i['t']/tot_b if tot_b > 0 else 1/len(gb); nt = round(ps*nuovo_tot); inc = (i['t']/p_b_v)*100
                    st.markdown(f'<div class="player-card" style="background-color: #e3f2fd; border: 3px solid #1e88e5;"><b>{n}</b><br><small><span class="stat-label">VAL:</span> <b>{max(0, nt-int(i["v"]))}</b> + <b>{int(i["v"])}</b> (VINC)<br><span class="stat-label">PESO:</span> {inc:.1f}% | <span class="stat-label">ANTE:</span> {int(i["t"])}</small></div>', unsafe_allow_html=True)
            with res_b:
                for n, i in dict_a.items():
                    ps = i['t']/tot_a if tot_a > 0 else 1/len(ga); nt = round(ps*nuovo_tot); inc = (i['t']/p_a_v)*100
                    st.markdown(f'<div class="player-card" style="background-color: #fbe9e7; border: 3px solid #e53935;"><b>{n}</b><br><small><span class="stat-label">VAL:</span> <b>{max(0, nt-int(i["v"]))}</b> + <b>{int(i["v"])}</b> (VINC)<br><span class="stat-label">PESO:</span> {inc:.1f}% | <span class="stat-label">ANTE:</span> {int(i["t"])}</small></div>', unsafe_allow_html=True)
            diff = nuovo_tot - tot_a; c_p1, c_p2 = st.columns(2)
            c_p1.markdown(f'<div class="patrimonio-box">PATR {sa}<br><h2>{int(p_a_v+diff)}</h2><small>PRIMA: {int(p_a_v)} | VAR: {int(diff):+d}</small></div>', unsafe_allow_html=True)
            c_p2.markdown(f'<div class="patrimonio-box">PATR {sb}<br><h2>{int(p_b_v-diff)}</h2><small>PRIMA: {int(p_b_v)} | VAR: {int(-diff):+d}</small></div>', unsafe_allow_html=True)

with t[5]: # TAGLI (CON STATS GOLDEN)
    ...
    if gt:
        ...
        st.markdown(f'<div class="cut-box" style="border:3px solid #ff4b4b;"><h3>RIMBORSO: {rimb} CREDITI</h3><hr><span class="stat-label">VALORE TOTALE:</span> <b>{int(v_p+v_v)}</b><br><span class="stat-label">PESO PATRIMONIO:</span> <b>{inc:.2f}%</b></div>', unsafe_allow_html=True)

with t[6]: # RIMBORSO CESSIONI (CON FIX SINTASSI)
    ...
    if not df_mercato.empty:
        ...
        h1.write("**NOME**"); h2.write("**SPESA**"); h3.write("**QUOT**"); h4.write("**CALCOLO**"); h5.write("**TOT**"); h6.write("**STATO**")
        for i, row in df_mercato.iterrows():
