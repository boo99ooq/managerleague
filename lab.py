...
# 1. SETUP UI
st.set_page_config(page_title="MuyFantaManager Golden V4.1", layout="wide", initial_sidebar_state="expanded")

# --- CSS INTEGRATO (Neretto 900 + Grafica Rose Premium + Box Golden) ---
st.markdown("""
<style>
...
# --- FUNZIONI SUPPORTO (Robuste per evitare quotazioni sparite) ---
def to_num(val):
...
def load_all():
    # Caricamento Rose e Quotazioni (Logica roseagg.py per non perdere dati)
    if not os.path.exists("rose_complete.csv"): return None, None, None, None, None
...
# TAB ROSE (Grafica Premium integrata)
with t[2]:
    if f_rs is not None:
        sq_r = st.selectbox("**SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="sq_rose")
        ...
        pal_piena = {'POR': '#F06292', 'DIF': '#81C784', 'CEN': '#64B5F6', 'ATT': '#FFF176', 'GIO': '#AB47BC'}
        ...
        for r in ['POR', 'DIF', 'CEN', 'ATT', 'GIO']:
            d_rep = df_team[df_team['Ruolo_N'] == r]; bg = pal_piena.get(r, '#fff')
            html_riass += f'<tr style="background-color:{bg};"><td>{r}</td><td>{len(d_rep)}</td><td>{int(d_rep["Prezzo_N"].sum())}</td><td>{int(d_rep["Quotazione"].sum())}</td></tr>'
        st.markdown(html_riass + '</tbody></table>', unsafe_allow_html=True)

# TAB SCAMBI (RIPRISTINO FORMULA COMPLESSA GOLDEN 3.2)
with t[4]:
    ...
    if f_rs is not None:
        ...
        if gA and gB:
            ...
            nt = round((ta+tb)/2) # Punto d'Incontro (Media)
            gap = ta - tb # GAP matematico
            
            st.markdown(f'<div class="punto-incontro-box">GAP: {gap:g} | MEDIA: {nt:g}</div>', unsafe_allow_html=True)
            
            ra, rb = st.columns(2)
            with ra: # Giocatori che entrano in A (arrivano da B)
            ...
            with rb: # Giocatori che entrano in B (arrivano da A)
            ...
            # Calcolo Patrimoni (Formula Golden 3.2)
...
# TAB BUDGET (Logica Golden con recupero mercato)
with t[1]:
    if f_rs is not None:
...
# Resto delle Tab (Classifiche, Vincoli, Tagli, Mercato) come in Golden 3.2...
# [Il resto del codice segue la logica identica al file gold.py inviato]
