    ...
    if f_pt is not None:
        with c1:
            ...
            st.dataframe(f_pt[['Posizione','Giocatore','P_N','FM']].sort_values('Posizione').style\
                ...
                .format({"P_N":"{:g}", "FM":"{:g}"}), hide_index=True, use_container_width=True)
    if f_sc is not None:
        with c2:
            ...
            st.dataframe(f_sc[['Posizione','Giocatore','P_S','GF','GS','DR']].sort_values('Posizione').style\
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        sel = st.multiselect("**FILTRA VOCI PATRIMONIO:**", voci, default=voci)
        bu['PATRIMONIO'] = bu[sel].sum(axis=1)
        st.dataframe(bu[['Squadra_N'] + sel + ['PATRIMONIO']].sort_values("PATRIMONIO", ascending=False).style\
            .background_gradient(cmap='YlOrRd', subset=['PATRIMONIO'])\
            .format({c: "{:g}" for c in bu.columns if c != 'Squadra_N'}), hide_index=True, use_container_width=True)

with t[2]: # ROSE
    if f_rs is not None:
        sq = st.selectbox("**SELEZIONA SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="rose_box")
...
with t[4]: # SCAMBI
    ...
    if f_rs is not None:
        col1, col2 = st.columns(2); lista_sq = sorted(f_rs['Squadra_N'].unique())
        with col1: sA = st.selectbox("SQUADRA A", lista_sq, key="sa_sc"); gA = st.multiselect("ESCONO DA A", f_rs[f_rs['Squadra_N']==sA]['Nome'].tolist(), key="ga_sc")
        with col2: sB = st.selectbox("SQUADRA B", [s for s in lista_sq if s!=sA], key="sb_sc"); gB = st.multiselect("ESCONO DA B", f_rs[f_rs['Squadra_N']==sB]['Nome'].tolist(), key="gb_sc")
        if gA and gB:
            def get_i(n):
            ...
            st.markdown(f'<div class="punto-incontro-box"><b style="color:{col_g};">GAP: {gap:g}</b><br><small>Media Scambio: {nt:g}</small></div>', unsafe_allow_html=True)
            r1, r2 = st.columns(2)
            with r1:
                for n, i in db.items():
                    nt_i = round((i['t']/tb)*nt) if tb>0 else nt
                    st.markdown(f'<div class="player-card" style="background-color:#e3f2fd; border:3px solid #1e88e5;"><b>{n}</b><br><small>VAL: {max(0, nt_i-int(i["v"])):g} + {i["v"]:g} (VINC)</small></div>', unsafe_allow_html=True)
            with r2:
                for n, i in da.items():
                    nt_i = round((i['t']/ta)*nt) if ta>0 else nt
                    st.markdown(f'<div class="player-card" style="background-color:#fbe9e7; border:3px solid #e53935;"><b>{n}</b><br><small>VAL: {max(0, nt_i-int(i["v"])):g} + {i["v"]:g} (VINC)</small></div>', unsafe_allow_html=True)

with t[5]: # TAGLI
    st.subheader("✂️ SIMULATORE TAGLI GOLDEN")
    if f_rs is not None:
        sq_t = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sq_t_box")
        gt = st.selectbox("GIOCATORE", f_rs[f_rs['Squadra_N'] == sq_t]['Nome'].tolist(), key="gioc_t_box")
        if gt:
...
with t[6]: # MERCATO
    ...
    with st.expander("➕ AGGIUNGI CESSIONE ESTERA"):
        sc = st.selectbox("Seleziona Giocatore:", [""] + sorted(f_rs['Nome'].unique()) if f_rs is not None else [""])
    ...
    if not df_mercato.empty:
        ...
        h1.write("**GIOCATORE**"); h2.write("**SPESA**"); h3.write("**QUOT**"); h4.write("**CALCOLO**"); h5.write("**TOT**"); h6.write("**STATO**")
        for idx, row in df_mercato.iterrows():
            col1, col2, col3, col4, col5, col6 = st.columns([2, 1, 1, 2, 1, 1])
            with col1: st.markdown(f"**{row['GIOCATORE']}**<br><small>{row['SQUADRA']}</small>", unsafe_allow_html=True)
            with col2: st.write(f"{row['SPESA']:g}")
            with col3: st.write(f"{row['QUOT']:g}")
            with col4: st.markdown(f"<small>50%:{row['RIMB_BASE']:g}+V:{row['VINCOLO']:g}</small>", unsafe_allow_html=True)
            with col5: st.write(f"**{row['TOTALE']:g}**")
            with col6:
        ...
        for i, (squadra, data) in enumerate(sq_stats.iterrows()):
            with cols_riep[i % 4]: st.markdown(f'<div class="refund-box"><small>{squadra}</small><br><b>+{data["TOT_GEN"]:g}</b><br><hr style="margin:5px 0; border:0; border-top:1px solid #ccc;"><span class="text-ufficiale">Uff:{data["UFFICIALE"]:g}</span><br><span class="text-probabile">Prob:{data["PROBABILE"]:g}</span></div>', unsafe_allow_html=True)
