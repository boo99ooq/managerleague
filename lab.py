...
# CSS AGGRESSIVO: Colpisce le classi interne di Streamlit per le tabelle
st.markdown("""
<style>
    /* Forza neretto su tutto l'applicativo */
    html, body, [data-testid="stAppViewContainer"] *, .stMarkdown p { 
        font-weight: 900 !important; 
        color: #000 !important; 
    }
    
    /* Forza neretto SPECIFICO per le intestazioni e celle delle tabelle Streamlit */
...
# --- FUNZIONE MAGICA PER IL NERETTO NELLE TABELLE ---
def bold_style(df):
...
with t[2]: # ROSE
    if f_rs is not None:
        ...
        with c2: cerca_r = st.text_input("🔍 **CERCA GIOCATORE**", "").upper()
        df_r = f_rs.copy()
...
with t[3]: # VINCOLI
    if f_vn is not None:
        ...
        sq_v = st.selectbox("**FILTRA SQUADRA**", ["TUTTE"] + sorted(f_vn['Sq_N'].unique()), key="vinc_filter")
        df_v = f_vn if sq_v == "TUTTE" else f_vn[f_vn['Sq_N'] == sq_v]
        st.dataframe(bold_style(df_v[['Squadra', 'Giocatore', 'Tot_Vincolo', 'Anni_T']].sort_values('Tot_Vincolo', ascending=False)).format({"Tot_Vincolo":"{:g}"}), hide_index=True, use_container_width=True)

# ... (seguono le altre Tab come Scambi e Tagli che usano lo stesso stile bold_style)
