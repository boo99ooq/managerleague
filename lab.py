...
# CSS DEFINITIVO (Neretto 900 su tutto, tabelle incluse)
st.markdown("""
<style>
    /* Forza il neretto 900 su ogni elemento di testo, incluse le tabelle */
...
with t[1]: # BUDGET
    if f_rs is not None:
        ...
        cols_to_format = bu.select_dtypes(include=['number']).columns
        st.dataframe(bu[['Squadra_N'] + sel + ['TOTALE']].sort_values('TOTALE', ascending=False).style\
            ...
            .format({c: "{:g}" for c in cols_to_format}), hide_index=True, use_container_width=True)

# ... (le altre tab seguono la stessa logica di pulizia)
