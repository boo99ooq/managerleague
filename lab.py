...
# CSS INTEGRALE (Grassetto, bordi netti 3px e punto di incontro compatto)
st.markdown("""
<style>
...
with t[0]:
    ...
    if f_pt is not None:
        with c1:
            ...
            st.dataframe(f_pt[['Posizione','Giocatore','P_N']].sort_values('Posizione').style.background_gradient(subset=['P_N'], cmap='YlGn'), hide_index=True, use_container_width=True)
    if f_sc is not None:
        with c2:
...
with t[1]:
    if f_rs is not None:
        ...
        st.dataframe(bu[['Squadra_N'] + sel + ['PATRIMONIO TOTALE']].sort_values("PATRIMONIO TOTALE", ascending=False).style.background_gradient(cmap='YlOrRd', subset=['PATRIMONIO TOTALE']), hide_index=True, use_container_width=True)

# TAB SCAMBI (Fix NameError + Punto Incontro centrale/ristretto)
with t[4]:
        ...
        if ga and gb:
            ...
            with res_a:
                for n, i in dict_b.items():
                    ...
                    st.markdown(f'<div class="player-card" style="background-color: #e3f2fd; border: 3px solid #1e88e5;"><b>{n}</b><br><small>VAL: <b>{max(0, nt-int(i["v"]))}</b> + VINC: <b>{int(i["v"])}</b><br>PESO: {inc:.1f}% | ANTE: {int(i["t"])}</small></div>', unsafe_allow_html=True)
            with res_b:
                for n, i in dict_a.items():
                    ps = i['t']/tot_a if tot_a > 0 else 1/len(ga); nt = round(ps*nuovo_tot); inc = (i['t']/p_a_v)*100
                    st.markdown(f'<div class="player-card" style="background-color: #fbe9e7; border: 3px solid #e53935;"><b>{n}</b><br><small>VAL: <b>{max(0, nt-int(i["v"]))}</b> + VINC: <b>{int(i["v"])}</b><br>PESO: {inc:.1f}% | ANTE: {int(i["t"])}</small></div>', unsafe_allow_html=True)
...
with t[5]:
    ...
    if gt:
        ...
        st.markdown(f'<div class="cut-box" style="border:3px solid #ff4b4b;"><h3>RIMBORSO: {rimb} CREDITI</h3><hr>VAL TOT: {int(v_p+v_v)} | PESO PATR: {inc:.2f}%</div>', unsafe_allow_html=True)
