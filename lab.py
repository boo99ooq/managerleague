...
# CSS GOLDEN DEFINITIVO (Grassetto 900 e Gradienti)
st.markdown("""
<style>
...
with t[1]: # BUDGET (FIX VALUE ERROR)
    if f_rs is not None:
        ...
        # Formattazione sicura: applichiamo :g solo alle colonne numeriche esistenti
        num_cols = bu.select_dtypes(include=['number']).columns
        st.dataframe(bu.sort_values('TOTALE', ascending=False).style.background_gradient(cmap='YlOrRd', subset=['TOTALE']).format("{:g}", subset=num_cols), hide_index=True, use_container_width=True)

with t[2]: # ROSE
    if f_rs is not None:
        c1, c2 = st.columns([1, 2])
        with c1: sq_sel = st.selectbox("**SQUADRA**", ["TUTTE"] + sorted(f_rs['Squadra_N'].unique()))
        with c2: cerca = st.text_input("🔍 **CERCA CALCIATORE (TUTTA LA LEGA)**", "").upper()
        df_d = f_rs.copy()
...
with t[3]: # VINCOLI (MENU RIPRISTINATO)
    if f_vn is not None:
...
with t[4]: # SCAMBI (CON VALORI ANTE E PATRIMONI)
