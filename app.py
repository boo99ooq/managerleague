...
# Messaggi di Debug
if f3 is None: st.error("❌ File 'rose_complete.csv' non trovato o non leggibile!")
if f4 is None: st.error("❌ File 'vincoli.csv' non trovato o non leggibile!")
...
if f4 is not None and len(f4.columns)>=3:
    with t[4]:
        ...
        with v2:
            st.write("🏃 Dettaglio")
            u=st.selectbox("Scegli:",sorted(f4[v[0]].unique()),key="v")
