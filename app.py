...
# CSS POTENZIATO PER LE CARD E IL GRASSETTO
st.markdown("""
<style>
    html, body, [data-testid="stAppViewContainer"] * { font-weight: 900 !important; }
    
    /* Stile della Card Giocatore nella Sidebar */
    .search-card { 
    ...
    .stat-value { font-size: 1.1em; color: #1a1a1a; }
</style>
...
# --- SIDEBAR: RICERCA CON RIQUADRI SEPARATI ---
with st.sidebar:
    ...
    if f_rs is not None:
        ...
        for n in scelte:
            ...
            # Rendering del riquadro (Card)
            st.markdown(f"""
            <div class="search-card">
                <div style="display: flex; justify-content: space-between;">
                    <span style="font-size:1.2em; color:#1a73e8;"><b>{n}</b></span>
                    <span style="background:#eee; padding:2px 8px; border-radius:10px; font-size:0.7em;">{dr['Ruolo']}</span>
                </div>
                <div style="font-size:0.8em; color:#666; margin-bottom:10px;">{dr['Squadra_N']}</div>
                
                <div style="display: flex; flex-direction: column; gap: 5px; border-top: 1px solid #eee; pt: 10px;">
                    <div style="display: flex; justify-content: space-between;">
                        <span class="stat-label">Acquisto:</span> <b>{int(dr['Prezzo_N'])}</b>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                        <span class="stat-label">Vincolo:</span> <b>{int(val_vinc)} <small>({durata_vinc})</small></b>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-top:5px; padding-top:5px; border-top: 1px dashed #ddd;">
                        <span class="stat-label" style="color:#2e7d32;">Quot. Attuale:</span> <b style="color:#2e7d32; font-size:1.1em;">{int(dr['Quotazione'])}</b>
                    </div>
                    <div style="display: flex; justify-content: space-between; background:#f9f9f9; padding:5px; border-radius:5px; margin-top:5px;">
                        <span class="stat-label">Tot Reale:</span> <b>{int(dr['Prezzo_N'] + val_vinc)}</b>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
# --- MAIN APP ---
st.title("⚽ **MUYFANTAMANAGER GOLDEN V6.5**")
...
with t[2]: # ROSE
    if f_rs is not None:
        ...
        sq = st.selectbox("**SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="sel_sq_rose_v65")
...
with t[3]: # VINCOLI
    if f_vn is not None:
        ...
        sq_vinc_sel = st.selectbox("**SELEZIONA SQUADRA**", ["TUTTE"] + sorted(f_vn['Sq_N'].unique()), key="sel_sq_vinc_65")
        df_vinc_filtered = f_vn if sq_vinc_sel == "TUTTE" else f_vn[f_vn['Sq_N'] == sq_vinc_sel]
        st.dataframe(df_vinc_filtered[['Squadra', 'Giocatore', 'Tot_Vincolo', 'Anni_T']].sort_values('Tot_Vincolo', ascending=False).style.background_gradient(cmap='Purples', subset=['Tot_Vincolo']).format({"Tot_Vincolo": "{:g}"}).set_properties(**{'font-weight': '900'}), hide_index=True, use_container_width=True)
...
with t[4]: # SCAMBI
    ...
    if f_rs is not None:
        ...
        with c1:
            sa = st.selectbox("**SQUADRA A**", l_sq, key="sa_sc_65")
            ga = st.multiselect("**DA A**", f_rs[f_rs['Squadra_N']==sa]['Nome_N'].tolist(), key="ga_sc_65")
        with c2:
            sb = st.selectbox("**SQUADRA B**", [s for s in l_sq if s != sa], key="sb_sc_65")
            gb = st.multiselect("**DA B**", f_rs[f_rs['Squadra_N']==sb]['Nome_N'].tolist(), key="gb_sc_65")
        if ga and gb:
            def get_v(n):
...
with t[5]: # TAGLI
    ...
    if f_rs is not None:
        sq_t = st.selectbox("**SQUADRA**", sorted(f_rs['Squadra_N'].unique()), key="st_sq_fin_65")
        gi_t = st.selectbox("**GIOCATORE**", f_rs[f_rs['Squadra_N'] == sq_t]['Nome_N'].tolist(), key="st_gi_fin_65")
        if gi_t:
