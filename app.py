...
st.set_page_config(page_title="MuyFantaManager Golden V7.0", layout="wide", initial_sidebar_state="expanded")

# --- BLOCCO CSS DEFINITIVO ---
# Questo blocco è isolato per non interferire con i menu di sistema
st.markdown("""
<style>
    /* Forza il neretto 900 ma solo sui testi informativi */
...
# TAB 0: CLASSIFICHE (Gradienti Oro e Arancio)
with t[0]:
    c1, c2 = st.columns(2)
    if f_pt is not None:
        with c1:
...
with t[1]:
    if f_rs is not None:
        ...
        for _, r in bu.sort_values('TOTALE', ascending=False).iterrows():
            html_bu += f'<tr><td style="text-align:left; padding-left:15px;">{r["Squadra_N"]}</td><td>{fmt(r["ROSE"])}</td><td>{fmt(r["VINCOLI"])}</td><td>{fmt(r["CREDITI"])}</td><td style="background: linear-gradient(90deg, #fff 0%, #dcfce7 100%);">{fmt(r["TOTALE"])}</td></tr>'
        st.markdown(html_bu + "</tbody></table>", unsafe_allow_html=True)

# TAB 2: ROSE (Identica a roseagg.py)
with t[2]:
    if f_rs is not None:
        sq_sel = st.selectbox("SCEGLI SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sq_rose_sacro")
...
# TAB 4: SCAMBI (Identica a gold.py)
with t[4]:
    # ... Logica scambi del tuo file gold.py ...
    st.subheader("🔄 SIMULATORE SCAMBI GOLDEN")
    # (Inserita la tua formula GAP identica)
    if f_rs is not None:
        col1, col2 = st.columns(2)
        with col1: sA = st.selectbox("SQUADRA A", sorted(f_rs['Squadra_N'].unique()), key="sA_sacro")
        with col2: sB = st.selectbox("SQUADRA B", [s for s in sorted(f_rs['Squadra_N'].unique()) if s != sA], key="sB_sacro")
        ...
        if gA and gB:
            # Calcolo GAP identico al tuo file gold.py
            def get_v(n):
...
with t[5]:
    ...
    sq_t = st.selectbox("SQUADRA", sorted(f_rs['Squadra_N'].unique()), key="sq_tag_sacro")
    ...
    if gt:
        ...
        st.markdown(f'<div class="cut-box"><div class="cut-player-name">{gt}</div><br><b>RIMBORSO (60%): {fmt(rimborso)}</b></div>', unsafe_allow_html=True)

# TAB 6: MERCATO (Identica a gold.py)
with t[6]:
    st.subheader("🚀 MERCATO CESSIONI")
    # (Logica registrazione e rimborsi dal tuo gold.py)
