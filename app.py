...
# 1. SETUP UI
st.set_page_config(page_title="MuyFantaManager Golden Ultimate V5.0", layout="wide", initial_sidebar_state="expanded")

# --- CSS TOTALE (Incrocio perfetto tra lab(1) e roseagg) ---
st.markdown("""
<style>
...
# --- CARICAMENTO DATI (Integrazione Totale) ---
def load_all():
    def ld_std(f):
    ...
    if f_rs is not None:
        ...
        if f_qt is not None:
            ...
            if c_v_qt:
                ...
                f_rs = pd.merge(f_rs, f_qt[['Match_Nome', 'Ruolo_QT_Norm', c_v_qt]], left_on=['Match_Nome', 'Ruolo_Merge'], right_on=['Match_Nome', 'Ruolo_QT_Norm'], how='left')
...
# --- SIDEBAR (Standard lab(1)) ---
with st.sidebar:
    st.header("🔍 RICERCA")
    if f_rs is not None:
        cerca = st.multiselect("**CERCA CALCIATORE**", sorted(f_rs['Nome'].unique()))
        for n in cerca:
...
with t[0]: # CLASSIFICHE (Con Gradienti)
    ...
    if f_pt is not None:
        with c1:
            ...
            st.dataframe(bold_df(f_pt[['Posizione','Giocatore','P_N']]).background_gradient(subset=['P_N'], cmap='YlGn'), hide_index=True, use_container_width=True)
    if f_sc is not None:
        with c2:
            st.subheader("⚔️ SCONTRI")
            f_sc['P_N'] = f_sc['Punti'].apply(to_num)
            st.dataframe(bold_df(f_sc[['Posizione','Giocatore','Punti']]).background_gradient(subset=['Punti'], cmap='Blues'), hide_index=True, use_container_width=True)
...
with t[1]: # BUDGET (Con Multiselect)
    if f_rs is not None:
        ...
        bu['CREDITI'] = bu['Squadra_N'].map(bg_ex).fillna(0); bu['RECUPERO'] = bu['Squadra_N'].map(rim_u).fillna(0)
        sel = st.multiselect("**VOCI:**", ['SPESA ROSE', 'SPESA VINCOLI', 'CREDITI', 'RECUPERO'], default=['SPESA ROSE', 'SPESA VINCOLI', 'CREDITI', 'RECUPERO'])
        if sel:
            bu['TOTALE'] = bu[sel].sum(axis=1)
            st.dataframe(bold_df(bu[['Squadra_N'] + sel + ['TOTALE']]).background_gradient(subset=['TOTALE'], cmap='YlOrRd').format("{:g}"), hide_index=True, use_container_width=True)

with t[2]: # ROSE (GOLDEN roseagg.py)
    if f_rs is not None:
        ...
        with c4: st.markdown(f'<div class="stat-card" style="border-color:#9c27b0;">👶 GIOVANI<br><b style="font-size:1.5em;">{len(df_team[df_team["Ruolo_N"]=="GIO"])}</b></div>', unsafe_allow_html=True)
        ...
        ord_type = st.radio("**ORDINA:**", ["REPARTO", "PREZZO"], horizontal=True)
...
with t[3]: # VINCOLI
    if f_vn is not None:
        ...
        st.dataframe(bold_df(df_v[['Squadra', 'Giocatore', 'Tot_Vincolo', 'Anni_T']]), hide_index=True, use_container_width=True)
...
with t[5]: # TAGLI
    if f_rs is not None:
        ...
        if gt:
            ...
            st.markdown(f'<div class="stat-card" style="border-color:#ff4b4b;"><h3>{gt}</h3>RIMBORSO: {round((v_a+vv)*0.6):g}</div>', unsafe_allow_html=True)
...
with t[6]: # MERCATO
    ...
    with st.expander("➕ AGGIUNGI"):
        ...
        if st.button("INSERISCI") and sc != "":
            info = f_rs[f_rs['Nome'] == sc].iloc[0]; vv = f_vn[f_vn['Giocatore_Match']==super_clean(sc)]['Tot_Vincolo'].sum() if f_vn is not None else 0
            pd.concat([df_mercato, pd.DataFrame([{"GIOCATORE": sc, "SQUADRA": info['Squadra_N'], "TOTALE": ((info['Prezzo_N'] + info['Quotazione'])*0.5)+vv, "STATO": "PROBABILE"}])], ignore_index=True).to_csv(FILE_DB, index=False); st.rerun()
    if not df_mercato.empty:
        for idx, row in df_mercato.iterrows():
            ...
            with c4: 
                if row['STATO']=="PROBABILE" and st.button("✅", key=f"u_{idx}"): df_mercato.at[idx, 'STATO']="UFFICIALE"; df_mercato.to_csv(FILE_DB, index=False); st.rerun()
                if st.button("🗑️", key=f"d_{idx}"): df_mercato.drop(idx).to_csv(FILE_DB, index=False); st.rerun()
        sq_m = df_mercato.groupby(['SQUADRA','STATO'])['TOTALE'].sum().unstack(fill_value=0)
        sq_m['TOT'] = sq_m.get('UFFICIALE',0) + sq_m.get('PROBABILE',0); cols = st.columns(4); colori = ["bg-azzurro", "bg-verde", "bg-rosa", "bg-giallo"]
        for i, (sn, d) in enumerate(sq_m.sort_values('TOT', ascending=False).iterrows()):
            with cols[i%4]: st.markdown(f'<div class="refund-box-pastello {colori[i%len(colori)]}">{sn}<br><b>+{d["TOT"]:g}</b></div>', unsafe_allow_html=True)
