        ...
        with t[4]:
            ...
            if len(v_cs) >= 3:
                ...
                with vy:
                    ...
                    st_v = df_v.sort_values(dv, ascending=False).style.map(lambda x: 'background-color: #ffcdd2' if x == 1 else '', subset=[dv])
                    st.dataframe(style_c(st_v), hide_index=True, use_container_width=True)
else:
