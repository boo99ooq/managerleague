...
if any([d_sc is not None, d_pt is not None, d_rs is not None, d_vn is not None]):
        ...
        with c2:
            ...
            if d_pt is not None:
                ...
                for col in ['Punti Totali', 'Media']:
                    if col in d_pt.columns:
                        d_pt[col] = pd.to_numeric(d_pt[col].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
    ...
    with tabs[1]:
        ...
        if d_rs is not None:
            f_col = next((c for c in d_rs.columns if 'squadra' in c.lower()), d_rs.columns[0])
